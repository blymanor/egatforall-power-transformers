...
interface OLTCCompartmentModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const OLTCCompartmentModal = ({ isOpen, onClose, mode, data }: OLTCCompartmentModalProps) => {
  const [formData, setFormData] = useState({
    ...
    oltcCondition: mode === 'create' ? '' : (data?.oltcCondition || '')
  });
  ...
  const handleSave = () => {
    toast.success("บันทึกข้อมูลสำเร็จ", {
      description: "ข้อมูล OLTC Compartment ถูกบันทึกแล้ว",
    });
    onClose();
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold text-center">
            {mode === 'create' ? 'เพิ่มข้อมูล' : mode === 'edit' ? 'แก้ไขข้อมูล' : 'แสดงข้อมูล'} OLTC Compartment
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 p-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label>สภาพ OLTC Compartment :</Label>
              <Select 
                value={formData.oltcCondition} 
                onValueChange={(value) => handleInputChange('oltcCondition', value)}
                disabled={isReadOnly}
              >
                <SelectTrigger>
                  <SelectValue placeholder="เลือกสภาพ OLTC Compartment" />
                </SelectTrigger>
                <SelectContent>
...
export default OLTCCompartmentModal;
