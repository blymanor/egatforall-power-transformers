...
const ConservatorTankModal = ({ isOpen, onClose, mode, data }: ConservatorTankModalProps) => {
  const [formData, setFormData] = useState({
    ...
    tankCondition: mode === 'create' ? '' : (data?.tankCondition || '')
  });
  ...
  const handleSave = () => {
    toast.success("บันทึกข้อมูลสำเร็จ", {
      description: "ข้อมูล Conservator Tank ถูกบันทึกแล้ว",
    });
    onClose();
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold text-center">
            {mode === 'create' ? 'เพิ่มข้อมูล' : mode === 'edit' ? 'แก้ไขข้อมูล' : 'แสดงข้อมูล'} Conservator Tank
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 p-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder="เลือกสภาพ Tank" />
                </SelectTrigger>
                <SelectContent>
