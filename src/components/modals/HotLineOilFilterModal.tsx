...
const HotLineOilFilterModal = ({ isOpen, onClose, mode, data }: HotLineOilFilterModalProps) => {
  const [formData, setFormData] = useState({
    ...
    filterCondition: mode === 'create' ? '' : (data?.filterCondition || '')
  });
  ...
  const handleSave = () => {
    toast.success("บันทึกข้อมูลสำเร็จ", {
      description: "ข้อมูล Hot Line Oil Filter ถูกบันทึกแล้ว",
    });
    onClose();
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold text-center">
            {mode === 'create' ? 'เพิ่มข้อมูล' : mode === 'edit' ? 'แก้ไขข้อมูล' : 'แสดงข้อมูล'} Hot Line Oil Filter
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 p-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label>สภาพ Oil Filter :</Label>
              <Select 
                value={formData.filterCondition} 
                onValueChange={(value) => handleInputChange('filterCondition', value)}
                disabled={isReadOnly}
              >
                <SelectTrigger>
                  <SelectValue placeholder="เลือกสภาพ Oil Filter" />
                </SelectTrigger>
                <SelectContent>
