...
const NGRModal = ({ isOpen, onClose, mode, data }: NGRModalProps) => {
  const [formData, setFormData] = useState({
    ...
    ngrCondition: mode === 'create' ? '' : (data?.ngrCondition || '')
  });
  ...
  const handleSave = () => {
    toast.success("บันทึกข้อมูลสำเร็จ", {
      description: "ข้อมูล NGR ถูกบันทึกแล้ว",
    });
    onClose();
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold text-center">
            {mode === 'create' ? 'เพิ่มข้อมูล' : mode === 'edit' ? 'แก้ไขข้อมูล' : 'แสดงข้อมูล'} NGR
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 p-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select 
                value={formData.ngrCondition} 
                onValueChange={(value) => handleInputChange('ngrCondition', value)}
                disabled={isReadOnly}
              >
                <SelectTrigger>
                  <SelectValue placeholder="เลือกสภาพ NGR" />
                </SelectTrigger>
                <SelectContent>
