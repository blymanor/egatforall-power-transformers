...
const ThreePhaseImpedanceMeasurementModal = ({ isOpen, onClose, mode, data }: ThreePhaseImpedanceMeasurementModalProps) => {
  const [formData, setFormData] = useState({
    ...
    impedanceA: mode === 'create' ? '' : (data?.impedanceA || ''),
    impedanceB: mode === 'create' ? '' : (data?.impedanceB || ''),
    impedanceC: mode === 'create' ? '' : (data?.impedanceC || '')
  });
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 p-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label>Impedance Phase A :</Label>
              <div className="flex items-center space-x-2">
                <Input
                  value={formData.impedanceA}
                  onChange={(e) => handleInputChange('impedanceA', e.target.value)}
                  placeholder="กรอกค่า Impedance A"
                  readOnly={isReadOnly}
                />
            ...
            <div className="space-y-2">
              <Label>Impedance Phase B :</Label>
              <div className="flex items-center space-x-2">
                <Input
                  value={formData.impedanceB}
                  onChange={(e) => handleInputChange('impedanceB', e.target.value)}
                  placeholder="กรอกค่า Impedance B"
                  readOnly={isReadOnly}
                />
                <span className="text-sm">%</span>
              </div>
            ...
            <div className="space-y-2">
              <Label>Impedance Phase C :</Label>
              <div className="flex items-center space-x-2">
                <Input
                  value={formData.impedanceC}
                  onChange={(e) => handleInputChange('impedanceC', e.target.value)}
                  placeholder="กรอกค่า Impedance C"
                  readOnly={isReadOnly}
                />
                <span className="text-sm">%</span>
              </div>
