...
const LightningArresterModal = ({ isOpen, onClose, mode, data }: LightningArresterModalProps) => {
  const [formData, setFormData] = useState({
    ...
    arresterCondition: mode === 'create' ? '' : (data?.arresterCondition || '')
  });
  ...
  const handleSave = () => {
    toast.success("บันทึกข้อมูลสำเร็จ", {
      description: "ข้อมูล Lightning Arrester ถูกบันทึกแล้ว",
    });
    onClose();
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold text-center">
            {mode === 'create' ? 'เพิ่มข้อมูล' : mode === 'edit' ? 'แก้ไขข้อมูล' : 'แสดงข้อมูล'} Lightning Arrester
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 p-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label>สภาพ Lightning Arrester :</Label>
              <Select 
                value={formData.arresterCondition} 
                onValueChange={(value) => handleInputChange('arresterCondition', value)}
                disabled={isReadOnly}
              >
                <SelectTrigger>
                  <SelectValue placeholder="เลือกสภาพ Lightning Arrester" />
                </SelectTrigger>
                <SelectContent>
