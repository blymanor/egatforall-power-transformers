...
const OilDGAModal: React.FC<OilDGAModalProps> = ({
  isOpen,
  onClose,
  mode,
  data
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {mode === "add" ? "เพิ่มข้อมูล Oil DGA" : 
             mode === "view" ? "ดูข้อมูล Oil DGA" : 
             "แก้ไขข้อมูล Oil DGA"}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label>ITF [dyne/cm] :</Label>
              <Input 
                value={formData.itf} 
                onChange={(e) => setFormData(prev => ({...prev, itf: e.target.value}))} 
                readOnly={isReadOnly}
                placeholder="ระบุค่า ITF"
              />
            </div>

            <div>
              <Label>NN [mgKOH/g] :</Label>
              <Input 
                value={formData.nn} 
            ...
            <div>
              ...
              <Input 
                value={formData.pf20} 
                onChange={(e) => setFormData(prev => ({...prev, pf20: e.target.value}))} 
                readOnly={isReadOnly}
                placeholder="ระบุค่า PF20"
              />
            </div>
